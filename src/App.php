...
class App {
  ...
  */
  ...
  public function buildRoutes():static{
    ...
    $this->app->get('/list', function (Request $request, Response $response, array $args) {
      // Récupération des données
      // $taskArray = (new DatabaseHandler("todolist"))->read([], "tasks");
      $taskArray = (new Collection("tasks"))->read([]); // OU $taskArray = (new Collection("tasks"))->getMany([]);
    ...
    $this->app->get('/task/{idTask}', function(Request $request, Response $response, array $args) {
      ...
      if( isset($parameters["idTask"]) ){
        // Récupération des données
        // $oneTask = (new DatabaseHandler("todolist"))->readById($parameters["idTask"], "tasks");
        $oneTask = (new Collection("tasks"))->readByID($parameters["idTask"])[0];
      }
    ...
    $this->app->post('/task', function(Request $request, Response $response, array $args) {
      ...
      if( $dataHandler->verifyData($parameters, true) ){
        ...
        // Écriture en BDD
        $idTask = (new DatabaseHandler("todolist"))->create($task, "tasks");
      }else{
    ...
    $this->app->put('/task/{idTask}', function(Request $request, Response $response, array $args) {
      ...
      if( $dataHandler->verifyData($parameters) &&  isset($parameters["idTask"])){
        ...
        if( $updatesTask != []){
          ...
          $oneTask = (new DatabaseHandler("todolist"))->update($parameters["idTask"], $updatesTask, "tasks");
        }else{$success=1;}
      }else{$success=2;}
    ...
    $this->app->delete('/task/{idTask}', function(Request $request, Response $response, array $args) {
      ...
      if( isset($parameters["idTask"]) ){
        ...
        $oneTask = (new DatabaseHandler("todolist"))->delete($parameters["idTask"], "tasks");
      }else{$success=1;}
